<script lang="ts">
  ...
  const ACTION_BUTTONS: Partial<Record<Download['status'], { Icon: ComponentType; size: string }>> =
    {
      completed: { Icon: Icon.FolderOpen, size: 'size-4' },
      failed: { Icon: Icon.RotateCw, size: 'size-4' },
      downloading: { Icon: Icon.XCircle, size: 'size-4' }
    };
  ...
  {#snippet child({ props })}
    <button type="button" {onclick} {...props} class="flex w-full items-center gap-4">
      <Item.Media>
        ...
        >
          <Icon.Music class="size-8 text-primary" />
        </div>
      </Item.Media>
